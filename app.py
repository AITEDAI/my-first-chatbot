...
dalle_client = AzureOpenAI(
    api_key=os.getenv("AZURE_OAI_KEY"), # DALL-E 인선미 강사님꺼 가져왔는데, 에러가 난다
    api_version="2024-04-01-preview",
    azure_endpoint=os.getenv("AZURE_OAI_ENDPOINT") # DALL-E 인선미 강사님꺼 가져왔는데, 에러가 난다
)
...
st.set_page_config(page_title="루미나 (Lumina): 당신의 운명을 비추는 🔮", page_icon="🔮", layout="wide")

# 사이드바 설정
with st.sidebar:
    st.image("https://cdn-icons-png.flaticon.com/512/4712/4712109.png", width=100)
    st.title("🔮 Lumina")
    st.write("당신의 운명을 비추는 빛")
    
    st.markdown("---")
    mode = st.radio("서비스 선택", ["💬 타로 상담 (채팅)", "✋ 관상/손금 보기", "🎨 행운의 부적 만들기(유료)"])
